...
def build_affordances(
    ...
    *,
    ...
    for index, obj in enumerate(map_info.objects):
        ...
        if obj.movement == "STAY":
            identity_hints.append("stationary_entity")
        else:
            identity_hints.append("moving_entity")
        if (obj.text_ref or "").endswith("POKE_BALL"):
            identity_hints.extend(["pickup_like", "starter_choice_like"])
        ...
        affordances.append(
            _annotate_affordance(
                snapshot,
                {
                    "id": f"object:{index}",
                    "kind": "object",
                    "label": f"Interact with nearby entity at ({obj.x}, {obj.y})",
...
def _candidate_confidence(
    ...
    *,
    ...
    if affordance.get("kind") == "object" and nearby_choice_interaction:
        ...
        if {"pickup_like", "starter_choice_like"} & hints:
            base += 0.1
